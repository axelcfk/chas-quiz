...
export default function HardQuiz() {
  ...
  const shuffleAnswers = () => {
    const answers = [
      ...defaultQuiz.resultsHard[index].incorrect_answers,
      defaultQuiz.resultsHard[index].correct_answer,
    ];
  ...
  const handleButtonClick = (selectedOption) => {
    if (selectedOption === defaultQuiz.resultsHard[index].correct_answer) {
      setIsCorrect(true);
    }
  ...
  return (
    <div className="flex justify-center flex-col items-center mx-10 bg">
      ...
      <p>
        Question {index + 1} of {defaultQuiz.resultsHard.length}
      </p>

      <h2>{defaultQuiz.resultsHard[index].question}</h2>
        ...
        {shuffledOptions.map((option, i) => (
          <button
            ...
            className={`m-5 h-36 w-36 p-2 border-none font-semibold rounded-md ${
              buttonClicked
                ? option === defaultQuiz.resultsHard[index].correct_answer
